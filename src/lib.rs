    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        ...
        if result.len() > MAX {
            return Err(QueryListError(
                format!("too many query parameters, max allowed is {MAX}")
            ));
        }
...
mod tests {
    ...
    async fn test_query_list_enum() {
        let uri: http::Uri = "/?id=123&username=abc&id=345".parse().unwrap();
        let mut parts = http::Request::builder()
            .uri(uri).body(()).unwrap().into_parts().0;
